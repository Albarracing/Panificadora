...
// Función auxiliar para calcular el importe final con descuentos
const calcularImporteFinal = (
  precioUnitario,
  cantidad,
  descuentoPorArticulo,
  descuentoCliente
) => {
  const importeSinDescuento = cantidad * precioUnitario;
  const importeConDescuentoPorArticulo =
    importeSinDescuento - (importeSinDescuento * descuentoPorArticulo) / 100;
  return (
    importeConDescuentoPorArticulo -
    (importeConDescuentoPorArticulo * descuentoCliente) / 100
  );
};
      ...
      clientesArticulos.map(async (clienteArticulo) => {
        ...
        const articulosConImportes = await Promise.all(
          clienteArticulo.articulos.map(async (articulo) => {
            ...
            const articuloCliente = await ArticuloCliente.findOne({
              clienteId: cliente._id,
              articuloId: articuloInfo._id,
            });

            const descuentoPorArticulo = articuloCliente
              ? articuloCliente.descuentoPorArt
              : 0;

            const descuento = cliente.descuento || 0;

            // Usamos la nueva función para calcular el importe final
...
//             console.log(
//               `Descuento por artículo (${articuloInfo._id}): ${descuentoPorArticulo}`
//             );

//             // Calcular el importe sin descuento
//             const importeSinDescuento = cantidad * precioUnitario;

//             // Obtener el descuento específico del artículo
//             //const descuentoPorArticulo = articulo.descuentoPorArt || 0;

//             // Aplicar el descuento específico del artículo
//             const importeConDescuentoPorArticulo =
//               importeSinDescuento -
//               (importeSinDescuento * descuentoPorArticulo) / 100;

//             const descuento = cliente.descuento || 0;

//             console.log(
//               `Aplicando descuento: Artículo ${descuentoPorArticulo}%, Cliente ${descuento}%`
//             );
//             // Ahora aplica el descuento general del cliente sobre el importe ya descontado
//             const importeFinal =
//               importeConDescuentoPorArticulo -
//               (importeConDescuentoPorArticulo * descuento) / 100;

//             totalCliente += importeFinal;
//             console.log(
//               `Descuento por artículo (${articulo.articulo}): ${articulo.descuentoPorArt}`
//             );
//             console.log(
//               `Descuento general del cliente (${cliente._id}): ${cliente.descuento}`
//             );
...
//               cantidadActual + cantidad
//             );
//             console.log(`Importe sin descuento: ${importeSinDescuento}`);
//             console.log(
//               `Importe con descuento por artículo: ${importeConDescuentoPorArticulo}`
//             );
//             console.log(
//               `Importe final con descuento del cliente: ${importeFinal}`
//             );
//             return {
//               articuloId: articulo.articulo,
//               nombre: nombreProducto,
